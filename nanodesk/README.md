...
**实用优先的个人 AI 助手** — 让每天的工作流更高效。

> **核心理念**：*实用第一，轻量第二*。解决真实问题，不做功能堆砌。

## 简介

Nanodesk 基于 [nanobot](https://github.com/HKUDS/nanobot) 构建，是一款**实用优先**的本地桌面 AI 助手：

- ✅ **开箱即用** - 搜索、文件操作、浏览器自动化，常用功能直接可用
- ✅ **全天候运行** - 关闭屏幕后继续运行，不错过任何消息
- ✅ **Windows 桌面应用** - 图形化配置，系统托盘管理，嵌入 Python 打包
- ✅ **深度飞书集成** - 专为国内用户优化，飞书机器人体验最佳
- ✅ **代码透明** - 完全本地运行，数据自主，每一行代码都可审计

### 对比：为什么选择 Nanodesk？

| 特性 | OpenClaw | Nanodesk |
|------|----------|----------|
| **定位** | 功能最全（~40万行） | **实用够用**（~4千-8千行）|
| **安装** | npm + 复杂配置 | pip 或**单文件运行** |
| **飞书支持** | 一般 | **深度优化** |
| **关屏运行** | ✅ | 🔧 **设计中（即将支持）**|
| **代码可控** | 较难（Node 编译）| **Python，易读易改** |

**适合人群**：想要一个"听话、实用、可控"的个人助手，而非庞大复杂的框架。
